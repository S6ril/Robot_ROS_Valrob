...
Ici je me base sur le composant `L298N` avec 2 moteurs à CC. Les pins de connexion à la carte arduino sont définis dans le fichier `Config.h` du code arduino.
...
On devrait voir toutes les touches de la manette. En l’actionnant, on vérifie que ça fonctionne.
...
- À la ligne `<param name="dev" type="string" value="/dev/input/js0" />`, on modifie le numéro du Joystick.
- À la ligne `portArduino : "ACM0"`, on modifie le port de l’arduino.
